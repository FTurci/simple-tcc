...
tcc10B::tcc10B( int _N ) : tcc_base( _N )
{
  try { fc = g_ctx.props.get<double>( "tcc.fc_10B" ); }
  catch(...) {}
...
void tcc10B::Rings_gSP4(int n0, int n1, int n2)
...
void tcc10B::Rings_gSP5(int n0, int n1, int n2, int n3) 
...
void tcc10B::Rings_aSP5(int n0, int n1, int n2, int n3, int n4)
...
void tcc10B::Clusters_Get10B_C3v()
{ 
  int i, j, k, l, m;
  int sp1, sp2i, sp2j, sp5com[2];
  int trial9B[9], trial10B[10];
    ...
    for (j=i+1; j<nsp5c; ++j) { // second 7A
      ...
      for (k=j+1; k<nsp5c; ++k) {
        if (sp5c[k][5] == sp1) {  // check one spindle of sp5c_k is the common spindle of 9B
          ...
          if (flg1==0 || flg2==0) continue;
          trial10B[6]=trial9B[6];
          trial10B[7]=trial9B[7];
          trial10B[8]=sp5c[k][6];
          trial10B[9]=trial9B[8];
      ...
      if (sp5c[k][6] == trial9B[8]) {  // check one spindle of sp5c_k is the common spindle of 9B
          if (Bonds_BondCheck(sp5c[k][5], trial9B[6])==0) continue;  // check other spindle of sp5c_k is bonded to spindle sp2i of 9B
          if (Bonds_BondCheck(sp5c[k][5], trial9B[7])==0) continue;  // check other spindle of sp5c_k is bonded to spindle sp2j of 9B
          
          flg1=0;  // check sp2i and sp2j are in SP5 ring of sp5c_k
          flg2=0;
          for (l=0;l<5;l++) {
            if (sp5c[k][l]==trial9B[6]) {
              flg1=1;
              continue;
            }
            if (sp5c[k][l]==trial9B[7]) {
              flg2=1;
              continue;
            }
          }
...
int tcc10B::Find()
{
  ...
  Clusters_Get10B_C3v();  // find the 10B

  total_10B_parts=0;
  for(i=0; i<N; i++) {
    total_10B_parts+=sClu[i];
  }

  return total_10B_parts;
}
